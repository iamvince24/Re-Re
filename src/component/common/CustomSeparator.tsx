...
export default function CustomSeparator(props: CustomSeparatorProps) {
  ...
  useEffect(() => {
    ...
    if (allNotebookData[notebookIndex] !== undefined) {
      tempNotebookName =
        allNotebookData[notebookIndex].chapters === undefined ? null : allNotebookData[notebookIndex]?.name
      tempChapterName =
  ...
  return (
    <ThemeProvider theme={theme}>
      <Stack spacing={2}>
        <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb" color="primary">
          {isNotebookMode ? (
            <Link
              //   underline="hover"
              underline="none"
